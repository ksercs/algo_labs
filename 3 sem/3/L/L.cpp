...
class SuffixArray {
public :
    SuffixArray(string & _s) : n(s.SZ), s(_s) {
        suff.resize(n, 0);
        c.resize(n, 0);
        int classCnt = firstSort();
...
//        countPref(s);
    ...
    int operator [](int index) {
        ...
        for (int i = 0; i < n; ++i) {
            if (!used_c.count(c[suff[i]])) {
                used_c.insert(c[suff[i]]);
                if (used_c.SZ == index) {
                    return suff[i];
                }
            }
...
int main()
{
    ...
    if (ind == -1)  {
        ...
        cout << s.substr(ind, s.SZ) + s.substr(0, ind) << "\n";
    }
